...
function HeroVideo() {
  ...
  return <video key={src} src={src} poster={poster} width={w} height={h} autoPlay muted loop playsInline preload="metadata" aria-hidden="true" tabIndex={-1} ref={el => { if (el) { el.muted = true; const p = el.play(); if (p && p.catch) p.catch(() => {}); } }}></video>;
}
...
function Hero() {
  ...
  const art = <div className="hero4-artlayer" aria-hidden="true">
      {LGFX_REDUCED ?
      <picture>
        <source media="(max-width:1199px)" srcSet="assets/hero-poster-mobile.png" width="1080" height="1920"></source>
