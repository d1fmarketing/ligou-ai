...
export function projectSalesLead(row) {
  ...
  return {
    ...
    contactConfirmed:row.contact_confirmed===true,canFollowUp:row.contact_confirmed===true&&row.followup_consent===true,
    consentChannel:typeof row.followup_consent_evidence?.channel==='string'?row.followup_consent_evidence.channel:'',
    updatedAt:row.updated_at,createdAt:row.created_at,
  };
}
export async function loadSalesLeads(client) {
  const {data,error}=await client.from('sales_leads').select('session_id,fields,contact_confirmed,followup_consent,followup_consent_evidence,created_at,updated_at').order('updated_at',{ascending:false}).limit(100);
