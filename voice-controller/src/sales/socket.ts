...
export async function attachSalesSocket(row:WorkerSession,store:SalesStore,stop:(reason:string)=>Promise<void>,socketFactory=(url:string,options:any)=>new WebSocket(url,options)):Promise<SalesSocket> {
 ...
 let closed=false, acknowledged=false, conversationStarted=false;
 ...
 ws.addEventListener('message',(event:any)=>{
   queue=queue.then(async()=>{
    ...
    if(message.type==='input_audio_buffer.speech_started'||message.type==='response.created')conversationStarted=true;
 ...
 return {send,greet(){if(!closed&&!conversationStarted){conversationStarted=true;send({type:'response.create'});}},close(){closed=true;try{ws.close();}catch{}}};
}
