...
function fixture(options:any={}){
 ...
 const deps:any={client,apiKey:'synthetic-openai-key',openTimeoutMs:10,closeTimeoutMs:5,cleanupTimeoutMs:5,
  ...
  createSession:async(input:any,dependencies:any)=>{creationCalls++;order.push('create');expect(input.responses).toMatchObject({model:'gpt-6-astra',reasoning:{effort:'low'}});return options.create?options.create(input,dependencies):{sessionId,sdp:'provider-answer',expiresAt};},
