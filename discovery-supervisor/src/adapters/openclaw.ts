...
function discoveryPrompt(job: WorkerJob): string {
  ...
  return [
    ...
    "Success requires submit_discovery_result to return accepted:true.",
    ...
    "schema_version must equal company_discovery.result.v1.",
    "source_snapshots must be the complete ordered array of fetched snapshot objects, copied byte-for-byte with no added, removed, renamed, or rewritten fields.",
    "Every candidate_facts item has exact keys: claim_class, claim_type, normalized_value, evidence_refs, contradictions, uncertainty.",
    "Descriptive claim_type is one of business_name, business_description, public_phone, public_email, public_address, public_website; normalized_value is a string.",
    "Operational facts use claim_class operational and claim_type service. normalized_value has exact keys service_type, service_names, public_price, duration_minutes.",
    "service_type is lowercase snake_case; service_names is a nonempty string array; public_price is null or {amount as decimal string with two digits, currency as three uppercase letters, qualifier as exact or starting_at}; duration_minutes is an integer or null.",
    "Safety facts use claim_class safety_critical, claim_type emergency, and normalized_value {guidance:string}.",
    "evidence_refs contains unique zero-based integers into source_snapshots. contradictions and uncertainty are string arrays.",
    "missing_questions, contradictions, and uncertainty are string arrays. Write missing_questions in Portuguese.",
    "Never infer owner-private prices, discounts, negotiation authority, approvals, or actions.",
    "Never return prose instead of obtaining accepted:true.",
