...
> 10. Sequência vigente: F-1 teardown → F0 consolidar → F1 voz+Hermes supervisor → F2 poderes+calendar → F3 EC2 → F4 skills/autonomia → F5 onboarding → F6 Twilio.

**Data:** 18/08/2026 · **Status:** HISTÓRICO com emenda acima; execução governada por `PLANO-MVP-2026-08-18.md`. Supersede `DECISAO-FINAL-O-FUNCIONARIO-2026-08-17.md` (parte de stack) e demove o pacote do Codex (worktree `ligou-architecture`) a **rulebook de segurança** — deixa de ser blueprint de plataforma.
...
| **Backup** | Snapshot EBS diário + export cognitivo com manifesto HMAC obrigatório → **S3** por tenant; restore descartável + rollback automático | ~$1–2 | A EC2 pode morrer; memória/skills/sessões voltam sem restaurar model-auth ou credenciais de negócio |
