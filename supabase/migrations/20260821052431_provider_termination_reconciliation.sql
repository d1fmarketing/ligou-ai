-- Provider termination is independently reconcilable even when session budget
-- reservation failed before the provider call could be rejected.
alter table public.calls
  add column if not exists provider_termination_reconcile_attempts integer not null default 0,
  add column if not exists provider_termination_reconcile_after timestamptz not null default now(),
  add column if not exists provider_termination_reconcile_lease_until timestamptz,
  add column if not exists provider_termination_reconcile_worker text;
