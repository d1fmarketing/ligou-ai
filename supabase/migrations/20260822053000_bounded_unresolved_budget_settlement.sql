...
-- permanently, and settle_call_budget refuses to settle in that state. The
-- reservation then holds its full ceiling against the tenant's daily budget
-- forever: five such reservations were already holding 6.00 of a 15.00 daily
-- budget in production.
--
-- This adds the bounded terminal path. It is deliberately NOT a release to zero:
-- the reservation settles at the same rate it was priced at, for the duration the
-- call durably recorded, so an unresolvable call is charged an honest estimate
-- rather than being forgiven. Every guard that makes settlement safe is enforced
...
begin
  ...
  -- Only for genuinely unresolvable usage: a resolvable call must go through
  -- settle_call_budget so it is charged its real cost.
  ...
  -- The provider must be done with the call, or we could still accrue cost.
  if coalesce(v_call.provider_termination_state, 'unknown') not in ('confirmed','not_required') then
...
-- The reconciliation worker needs the attempt count and the reserved ceiling to
-- decide when the bounded path applies and at what rate to price it. Attempts are
-- reported post-increment so the worker sees the attempt it is performing.
