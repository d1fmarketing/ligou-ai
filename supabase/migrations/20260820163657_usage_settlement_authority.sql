-- Usage resolution is settlement authority, so new calls fail closed until
-- either a validated provider receipt resolves usage or a pre-provider path
-- proves that provider usage was not applicable.
alter table public.calls
  alter column provider_usage_state set default 'unknown',
  add column if not exists provider_usage_evidence jsonb;

-- The prior migration treated every missing provider call id as proof that no
-- provider usage existed. A missing id is ambiguous. Preserve not_applicable
-- only for the one historical reason that proves no provider request could
-- have been made; all other legacy no-id classifications become unknown.
update public.calls c
set provider_usage_state = case
  when c.openai_call_id is null and c.provider_termination_reason = 'openai_key_missing'
    then 'not_applicable' else 'unknown'
end
where c.provider_usage_state = 'not_applicable';
...
begin
  ...
  -- Reconciliation already leases the reservation first. Keep the same
  -- reservation -> call lock order here to avoid an authority-check deadlock.
